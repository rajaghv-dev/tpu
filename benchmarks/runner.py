...
from observe.otel import get_instruments, get_tracer, init_otel, shutdown_otel
...
def run_experiment(
    ...
    # OTel bootstrap — no-op unless TPU_BENCH_OTEL=otlp|file. Metrics recorded
    # post-phases; per-phase spans are deferred to the OtelStackProbe refactor
    # (see ADR-016 risks). shutdown_otel() runs in the outer finally below.
    ...
    # ── OTel metrics (recorded once per experiment; no-op unless enabled) ──
    for t_ms in all_latency_ms:
        instruments["latency_ms"].record(t_ms, metric_attrs)
    instruments["throughput_samples_sec"].record(tp.mean_samples_sec, metric_attrs)
